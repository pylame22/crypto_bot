...
class DepthData:
    ...
    def is_valid_first_event(self, symbol: str) -> bool:
        first_event = next(iter(self.depth_events[symbol].values()))
        return first_event.first_update_id <= self.depth_results[symbol].last_update_id <= first_event.final_update_id
...
class DepthService:
    ...
    def _calculate_depth(self, data: DepthChangeSchema) -> None:
